...
<? if ($erreur_url) require("tpl/erreur_url.php"); else if ($erreur_var) require("tpl/erreur_var.php"); else if ($erreur_cookies) require("tpl/erreur_cookies.php"); else { ?>
		<div class="navi"><a href=".">Accueil</a> &gt; <a href="indicatifs/<?=urlencode($cursus)?>">Enseignement <?=Geo::$COURSE[$cursus]?></a> &gt; <a href="depts/<?=urlencode($cursus)?>/<?=urlencode(Geo::$DEPT[$dept]["ind"])?>/">Indicatif <?=htmlspecialchars(Geo::$DEPT[$dept]["ind"])?></a> &gt; <a href="villes/<?=urlencode($cursus)?>/<?=urlencode($dept)?>/"><?=htmlspecialchars($dept)?> - <?=htmlspecialchars(Geo::$DEPT[$dept]["nom"])?></a> &gt; <a href="etblts/<?=urlencode($cursus)?>/<?=urlencode($c_id)?>/"><?=htmlspecialchars($cp)?> - <?=htmlspecialchars($c_nom)?></a> &gt; <a href="profs2/<?=urlencode($e_id)?>/"><span class="etab"><?=htmlspecialchars($e_nom)?></span></a> &gt; <a href="notes/<?=urlencode($p_id)?>/"><?=htmlspecialchars($prenom)?> <span class="up"><?=htmlspecialchars($nom)?></span></a> &gt; <?=htmlspecialchars($title)?></div>
		<hr />
		<div>

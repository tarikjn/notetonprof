...
if (@strlen($expl[1]) > 0)
{
	...
	if ($rnb)
	{
		...
		$query =	"SELECT DISTINCT COUNT( notes.id ) AS notes, matieres.nom AS matiere, professeurs.nom, prenom, professeurs.id, UNIX_TIMESTAMP( MAX( notes.date ) ) AS lastdate , AVG(".Ratings::SQL_AVERAGE.") AS moy, AVG( FIND_IN_SET( 'pop', extra ) || FIND_IN_SET( 'in', extra ) ) AS pop ".

...
if (@strlen($expl[1]) > 0)
{
	...
	if (isset(Geo::$COURSE[$cursus]))
	{
		...
		$query =
			"SELECT etablissements.id, etablissements.nom, secondaire, villes.nom AS commune, AVG( interet + pedagogie + connaissances ) / 3 AS moy, COUNT( DISTINCT notes.id ) AS notes ".
